...
if ($res5) {
    ...
    <select type="text" class="form-control" id="in_fo_id" name="in_fo_id">
                ...
                <option value="<?=htmlentities($food['fo_id'],ENT_COMPAT,'UTF-8');?>" <?php if ($food['fo_id'] == $foodInstruction['in_fo_id']){ echo ' selected="selected"'; } ?>><?=$food['fo_type'] ?></option>
                <?php ;}?>
            </select></div>
    <div class="col-2"><input type="text" name="in_quantity" class="form-control" value='<?= $foodInstruction['in_quantity'];?> '>    </div>
    <div class="col-1"> <p> g </p></div>
    <div class="col-md-4 row">
           <div class="col-sm-3 col-md-5"><input type="submit" name="<?php echo 'modifyInstruction'.$foodInstruction['in_id'];?>" class="btn btn-primary btn-sm" value="Modifier"></div>
    <div class="col-sm-3 col-md-5"><input type="submit" name="<?php echo 'deleteInstruction'.$foodInstruction['in_id'];?>" class="btn btn-primary btn-sm" value="Supprimer"></div>
    </div>
...
<a class="btn btn-primary btn-sm js-button-ajouter-food col-4 mt-5" data-bs-toggle="collapse" href="#collapseAddFoodInstruction" role="button" aria-expanded="false" aria-controls="collapseExample" id="bouton-addFood">
    Ajouter une instruction
  </a>
  <div class="collapse mt-3" id="collapseAddFoodInstruction" >
  <form name="addFoodInstruction" method="POST">
<div class="row g-1">           
    <div class="col-sm-5">
    <select type="text" class="form-control" id="in_fo_id" name="in_fo_id">
                <?php foreach ($foods as $food) {?>
        ...
        <a class="btn btn-primary btn-sm js-button-ajouter-food col-4 mt-5" data-bs-toggle="collapse" href="#collapseAddFood" role="button" aria-expanded="false" aria-controls="collapseExample" id="bouton-addFood">
    Ajouter un nouveau type de nourriture
  </a></div>
