    ...
    <div class="border p-3 rounded mt-3">
            ...
            <div>
                <form method="POST">
                    <div class="row my-1 g-1">
                        ...
                        <div class="col-2">
                            <input type="text" name="fe_quantity" class="form-control">    
                        </div>
                        <div class="col-1"> 
                            <p> g </p>
                        </div>
                        <div class="col-2">
                            <input type="date" name="fe_date" class="form-control"> 
                        </div>
                        <div class="col-2">
                            <input type="time" name="fe_time" class="form-control">    
                        </div>
        ...
        <?php foreach($foodGiven as $foodGivenItem) {
            if (isset($_POST['deleteFoodGiven'.$foodGivenItem['fe_id']])) { ?>
                <!--empecher le renvoi du formulaire à l'actualisation de la page-->
                <script> location.replace(document.referrer); </script>
                <?php $res = deleteFoodGiven($pdo, $foodGivenItem['fe_id']);
                if ($res) {
            ...
            <div>
                <div class="row border-top pt-1">
                    <div class="col-3">
                        <p><?= htmlentities($foodGivenItem['fo_type']); ?></p>
                    </div>
                    <div class="col-2">
                        <p><?= htmlentities($foodGivenItem['fe_quantity']); ?> g</p>
                    </div>
                    <div class="col">
                        <p><?= date('d/m/y',strtotime(htmlentities($foodGivenItem['fe_date']))); ?>   <?= date('H:i',strtotime(htmlentities($foodGivenItem['fe_time']))); ?></p>
                    </div>
                    <?php if ($_SESSION['user']['us_role']==='employe') {?>
                        <div class="col">
                            <form method="POST">
